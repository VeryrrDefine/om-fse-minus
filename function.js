function getOFCost(x) {
  return EN(10)
    .pow((x + 1) * (game.omegaFactorize[x] + 1))
    .div(game.incrementer[3][0].prod);
...
function getFactorialGain(x = game.diagonalUp[0]) {
  ...
  return EN.factorial(x.div(1000000))
    .pow(1 + game.fupgrades.includes(6))
    .times(getIncMult() >= 10 ? omegaFactorMult : 1)
    .floor();
}

function getNextFactorial() {
  if (
    getFactorialGain(game.diagonalUp[0].add(1)).gte(getFactorialGain().add(0.5))
  ) {
    return 1;
  }
  let doubler = 524288;
  ...
  while (doubler >= 0.75) {
    ...
    adder += doubler;
    if (
      getFactorialGain(game.diagonalUp[0].add(adder)).gte(
        getFactorialGain().add(0.5),
      )
    ) {
      adder -= doubler;
    }
...
function getDiagSlowdown() {
  let k = new EN(
    game.pupgrades.includes(4) ? game.products.add(1e10).log10().div(10) : 1,
  );
